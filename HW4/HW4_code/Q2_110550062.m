...
s = 0.72;
for idx = 1:4
    tmp = D(idx, 2)+D(idx, 3)*((s-x(idx)) + (s-x(idx+1)))+D(idx, 4)*( (s-x(idx))*(s-x(idx+1)) + (s-x(idx+1))*(s-x(idx+2)) + (s-x(idx+2))*(s-x(idx)));
    tmpd = abs(true_a - tmp);
    if(tmpd<dif)
        dif = tmpd;
        est = tmp;
...
s = 0.5;
for idx = 1:3
    tmp = D(idx, 2)+D(idx, 3)*((s-x(idx))+(s-x(idx+1))) + D(idx, 4)*((s-x(idx))*(s-x(idx+1))+(s-x(idx+1))*(s-x(idx+2))+(s-x(idx+2))*(s-x(idx)))+ D(idx, 5)*((s-x(idx))*(s-x(idx+1))*(s-x(idx+2)) + (s-x(idx+1))*(s-x(idx+2))*(s-x(idx+3))+(s-x(idx+2))*(s-x(idx+3))*(s-x(idx)) + (s-x(idx+3))*(s-x(idx))*(s-x(idx+1)) );
    tmpd = abs(true_c - tmp);
    if(tmpd<dif)
        dif = tmpd;
        est = tmp;
